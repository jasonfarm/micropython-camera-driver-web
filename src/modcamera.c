...
static mp_obj_t camera_rawgma(mp_obj_t what){
    sensor_t * s = esp_camera_sensor_get();
    if (!s) {
        ESP_LOGE(TAG, "White Balance Failed");
        return mp_const_false;
    ...
    int val = mp_obj_get_int(what);
    s->set_raw_gma(s, val); // 0 or 1 (default 0).
    return mp_const_none;
}

static MP_DEFINE_CONST_FUN_OBJ_1(camera_rawgma_obj, camera_rawgma);

static mp_obj_t camera_start_camera_server(mp_obj_t what){
    startCameraServer();
    return mp_const_none;
}

static MP_DEFINE_CONST_FUN_OBJ_1(camera_start_camera_server_obj, camera_start_camera_server);


static mp_obj_t camera_start_camera_server_car(mp_uint_t n_pos_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    enum {
       ARG_wifi_addr,
       ARG_qplb,
       ARG_qplf,
    ...
    static const mp_arg_t allowed_args2[] = {
            { MP_QSTR_wifi_addr,    MP_ARG_KW_ONLY | MP_ARG_OBJ, {.u_obj = MP_ROM_QSTR(MP_QSTR_tt)} },
            { MP_QSTR_qplb,         MP_ARG_KW_ONLY  | MP_ARG_INT,   {.u_int = 12} },
            { MP_QSTR_qplf,         MP_ARG_KW_ONLY  | MP_ARG_INT,   {.u_int = 13} },
            { MP_QSTR_qprb,              MP_ARG_KW_ONLY  | MP_ARG_INT,   {.u_int = 14} },
            { MP_QSTR_qprf,              MP_ARG_KW_ONLY  | MP_ARG_INT,   {.u_int = 15} },
            { MP_QSTR_led,              MP_ARG_KW_ONLY  | MP_ARG_INT,   {.u_int = 4} },
    };
    mp_arg_val_t args[MP_ARRAY_SIZE(allowed_args2)];
    mp_arg_parse_all(n_pos_args-1, pos_args+1, kw_args, MP_ARRAY_SIZE(allowed_args2), allowed_args2, args);
    const char* wifi_addr = mp_obj_str_get_str(args[ARG_wifi_addr].u_obj);
    int  qplb = args[ARG_qplb].u_int;
    int  qplf = args[ARG_qplf].u_int;
    int  qprb = args[ARG_qprb].u_int;
    int  qprf = args[ARG_qprf].u_int;
    int  qled = args[ARG_led].u_int;
    startCameraServerCar(wifi_addr, qplb, qplf, qprb, qprf, qled);
    return mp_const_true;
}


static MP_DEFINE_CONST_FUN_OBJ_KW(camera_start_camera_server_car_obj, 1, camera_start_camera_server_car);
...
static const mp_rom_map_elem_t camera_module_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_rawgma), MP_ROM_PTR(&camera_rawgma_obj) },

    { MP_ROM_QSTR(MP_QSTR_start_camera_server), MP_ROM_PTR(&camera_start_camera_server_obj) },
    { MP_ROM_QSTR(MP_QSTR_start_camera_server_car), MP_ROM_PTR(&camera_start_camera_server_car_obj) },
